<template>
  <q-header class="text-primary q-pl-sm q-pr-lg q-pt-lg header-container">
    ...
    <!-- <img :src="proximaNameLogo" alt="My Icon" class="tmpl-icon" /> -->
  </q-header>
</template>
...
import proximaNameLogo from '@/assets/proxima_name_logo.svg';
...
export default defineComponent({
  ...
  setup() {
    ...
    const pageTitle = ref(route.meta.title || route.name || 'tmpl');
    const pageDescription = ref(route.meta.description || '');

    const processedTitle = computed(() => {
      return pageTitle.value.replace(/\|?\s*tmpl\s*$/, '').trim();
    });
    ...
    watch(route, (newRoute) => {
      pageTitle.value = newRoute.meta.title || newRoute.name || 'tmpl';
